...
def get_user_preferences(user_id):
    ...
    video_percentage = user_row['video_percentage'].values[0]
    audio_percentage = user_row['audio_percentage'].values[0]
    text_percentage = user_row['text_percentage'].values[0]

    return video_percentage, audio_percentage, text_percentage
...
def train(model_name, params, enrolled_courses):
    ...
    if model_name == models[5]: nmf = nmf_train(n_factors=params['nmf_factors'], n_epochs=params['nmf_epochs'],
                                                verbose=False)
    if model_name == models[6]:
        ...
        if (params['ncf_val_split'] == 0.1) & (params['ncf_batch_size'] == 512) & (params['ncf_epochs'] == 20):
            ...
            ds_train, ds_val = ncf_build_train_val_dataset(df, val_split=params['ncf_val_split'],
                                                           batch_size=params['ncf_batch_size'])
...
def predict(model_name, active_user, params, user):
    ...
    # Get the user's preferences
    video_percentage, audio_percentage, text_percentage = get_user_preferences(user)

    format_pref_dict = {"VIDEO": video_percentage, "AUDIO": audio_percentage, "TEXT": text_percentage}

    if model_name == "HYBRID":
        weights = {"Course Similarity": 0.5, "Clustering": 0.8,
                   "Clustering with PCA": 0.2}  # change this with actual model names and weights
        res = hybrid_recommender(active_user, weights, params)

    # Course Similarity model (Similarity Matrix, content based)
    if model_name == models[0]:
        n_rec = params['n_rec_sim']
        res = course_similarity_model(idx_id_dict, id_idx_dict, enrolled_course_ids, load_course_sims().to_numpy(),
                                      params['t_rec_sim'])
    ...
    if res:
        ...
        res_dict['FORMAT'] = courses_df['FORMAT'][courses_df['COURSE_ID'].isin(courses)].values
        res_dict['SCORE'] = scores
        res_df = pd.DataFrame(res_dict, columns=['USER', 'COURSE_ID', 'TITLE', 'FORMAT', 'SCORE'])
        res_df.sort_values(by=['SCORE'], ascending=False, inplace=True)

        total_rec = len(res_df)
        format_rec = {format_: round(total_rec * percent) for format_, percent in format_pref_dict.items()}

        final_rec_df = pd.DataFrame()
        for format_, num_rec in format_rec.items():
            print("num_rec", num_rec)
            rec_for_format = res_df[res_df['FORMAT'] == format_][:num_rec]
            final_rec_df = pd.concat([final_rec_df, rec_for_format])

        return final_rec_df[:n_rec]
...
def course_similarity_model(idx_id_dict: dict, id_idx_dict: dict, enrolled_courses: list, sim_matrix,
                            t_rec: float) -> dict:
    """
...
# # KMeans clustering
def train_cluster(user_profiles, genre_matrix, enrolled_courses, pca: bool = False, exp_var: int = 80, n_clu: int = 15,
    ...
    # - flatten (sum up by column) and store as numpy array
    # - store new user profile vector as 'active_user'
    active_user = ((genre_matrix.copy().drop('TITLE', axis=1).set_index('COURSE_ID').loc[enrolled_courses]) * 3).sum()
...
def clustering(model, user_profiles: pd.DataFrame, rated_courses: pd.DataFrame, enrolled_courses: list) -> dict:
    ...
    sim_users = {
        'user': [user for user in user_cluster_dict if (user_cluster_dict[user] == au_cluster) & (user != active_user)]}
...
# # KNNBasic (surprise)
def knn_train(k: int = 40, min_k: int = 2, metric: str = 'msd', user_based: bool = False,
              verbose: bool = True) -> KNNBasic:
    """
...
def ncf_create(n_users: int, n_items: int,  # user and item number (= input dimensions)
               ...
               # latent dimentions for matrix factorization (mf) and multi-layer-perceptron (mlp)
               ...
               # list index is dense layer, list value is regularization strength for that dense layer
    ...
    # Results layer (combine last dense layer with mf and put through resulting vector through dense layer
    # 'interaction')
...
def ncf_build_train_val_dataset(df: pd.DataFrame, val_split: float = 0.1, batch_size: int = 512,
                                rs: int = 42) -> tf.data.Dataset:
    # Generate training and validation datasets
    n_val = round(df.shape[0] * val_split)  # number of validation rows
    if rs:
        x = df.sample(frac=1, random_state=rs).to_dict('series')
    else:
        x = df.to_dict('series')
...
def ncf_predict(model, enrolled_courses: list = []) -> dict:
    ...
    # - re transform item_ids from numbers to course names
...
def emb_create(n_user, n_item, n_user_latent_dim: int = 16, n_item_latent_dim: int = 16, reg_users: int = 1e-6,
               reg_items: int = 1e-6) -> keras.Model:
    """
    Build Neural Network for the embedding. Returns the complete model, and the models for user and item embeddings.
    Is called by emb_train()
    """
...
def emb_ds_create(df: pd.DataFrame, targets: list, scale: bool = True, tt_split: int = 0.8, val_split: int = 0.1,
                  batch_size: int = 512, rs: int = 42):
    """
    ...
    if rs:
        x_train = df.sample(frac=1, random_state=rs).to_dict('series')
    else:
        x_train = df.to_dict('series')
...
def emb_train(ds: tf.data.Dataset, df: pd.DataFrame = load_ratings(), epochs: int = 10):
    ...
    emb_model_all.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer=keras.optimizers.Adam(),
...
def emb_classifier_predict(model, enrolled_courses: list, df_regression: pd.DataFrame,
                           df_ratings: pd.DataFrame = load_ratings()) -> dict:
    """
    ...
    if not 'UserID' in knn_pred_df.columns:
        user_ids = pd.DataFrame([df_regression.loc[u, 'user'] for u in knn_pred_df['UserIdx'].values]).rename(
            {0: 'UserID'}, axis=1)
...
def load_user_format_weights(user_id):
    user_data_df = load_users()
    user_data = user_data_df.loc[user_data_df['user_id'] == user_id]
    return {
        'VIDEO': user_data['video_percentage'].values[0],
        'AUDIO': user_data['audio_percentage'].values[0],
        'TEXT': user_data['text_percentage'].values[0],
    }


def update_completed_courses(user_id, completed_course_id):
    ...
    # Get the current list of completed courses for this user
    current_courses = user_row['completed_courses'].values[0]

    # Create an empty list if there are no completed courses yet
    if pd.isnull(current_courses):
        current_courses = []
    else:
        # Otherwise, split the string of completed courses into a list
        current_courses = current_courses.split(';')

    # Add the new completed course if it's not already in the list
    if completed_course_id not in current_courses:
        current_courses.append(completed_course_id)

    # Convert the list back into a semicolon-separated string
    updated_courses = ';'.join(current_courses)

    # Update the user's row with the new string of completed courses
    df.loc[df['user_id'] == user_id, 'completed_courses'] = updated_courses

    # Write the DataFrame back to the CSV file
    df.to_csv(p, index=False)


def hybrid_recommender(user_id, weights, params):
    hybrid_scores = {}
    for model_name, weight in weights.items():
        print(f"Model: {model_name}, Weight: {weight}")
        if model_name in models[:2]:
            recs = predict(model_name, user_id, params, user_id)
            print(f"Recommendations from {model_name}:", recs)
            for index, row in recs.iterrows():
                if row['COURSE_ID'] not in hybrid_scores:
                    hybrid_scores[row['COURSE_ID']] = row['SCORE'] * weight
                else:
                    hybrid_scores[row['COURSE_ID']] += row['SCORE'] * weight
    print("Hybrid scores:", hybrid_scores)
    return dict(sorted(hybrid_scores.items(), key=lambda item: item[1], reverse=True))


def update_course_rating(user_id, course_to_rate, rating):
    # Create a DataFrame with the new rating data
    ...
    # Append the new rating to the existing ratings file
    with open('./data/ratings.csv', 'a') as f:
        new_rating.to_csv(f, header=False, index=False)

    print(f"Rating for course {course_to_rate} by user {user_id} has been updated successfully!")
